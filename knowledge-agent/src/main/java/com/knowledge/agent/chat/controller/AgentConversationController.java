package com.knowledge.agent.chat.controller;
...
public class AgentConversationController {
    ...
    @Autowired private ObjectProvider<AgentStreamingModelService> modelServiceProvider;
    @Autowired private TaskExecutor agentRunExecutor;

    @PostMapping
    public Result<AgentConversationBO> create(@Valid @RequestBody CreateConversationDTO request,
    ...
    public Result<List<AgentHistoryMessageBO>> messages(@PathVariable String conversationId) {
    ...
    public SseEmitter stream(@PathVariable String conversationId, @Valid @RequestBody SendAgentMessageDTO request) {
        AgentStreamingModelService model = modelServiceProvider.getIfAvailable();
        if (model == null) throw BusinessException.serviceUnavailable("小智模型尚未配置");
        String userId = UserContext.getUserId();
        AgentRunBO run = conversationService.start(userId, conversationId, request);
        if (run.replay()) throw BusinessException.conflict("相同请求已创建运行，请刷新会话记录");
        SseEmitter emitter = new SseEmitter(35_000L);
        agentRunExecutor.execute(() -> generate(emitter, model, run.runId(), request));
        return emitter;
    }

    private void generate(SseEmitter emitter, AgentStreamingModelService model, String runId, SendAgentMessageDTO request) {
        try {
            ...
            model.stream(new AgentModelRequest(system, prompt, "anonymous"), delta -> {
                answer.append(delta);

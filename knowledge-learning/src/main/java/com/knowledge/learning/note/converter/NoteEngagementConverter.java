...
public final class NoteEngagementConverter {
    ...
    public static NoteCommentBO toBO(NoteCommentDO source, String viewerUserId) {
        return new NoteCommentBO(source.getId(), source.getNoteId(), source.getUserId(),
                source.getParentCommentId(), source.getContent(), source.getUserId().equals(viewerUserId), source.getVersion(),
                source.getCreatedAt(), source.getUpdatedAt());
    }

    public static NoteCommentVO toVO(NoteCommentBO source) {
        return new NoteCommentVO(source.id(), source.noteId(), source.authorId(), source.parentCommentId(),
                source.content(), source.owned(), source.version(), source.createdAt(), source.updatedAt());
    }

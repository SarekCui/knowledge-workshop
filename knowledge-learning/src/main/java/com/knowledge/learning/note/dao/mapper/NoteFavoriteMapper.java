...
public interface NoteFavoriteMapper extends BaseMapper<NoteFavoriteDO> {

    @Delete("DELETE FROM note_favorite WHERE note_id = #{noteId} AND user_id = #{userId}")
    int deleteByUser(@Param("noteId") String noteId, @Param("userId") String userId);

    @Select("SELECT COUNT(*) FROM note_favorite WHERE note_id = #{noteId} AND user_id = #{userId}")

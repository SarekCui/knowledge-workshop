...
- 正常播放时每 **10 秒**上报一次；位置未变化且无新增观看区间时跳过心跳。
...
采用Jellyfin Web的定时心跳与暂停/停止事件保存组合思路，不承诺毫秒级实际恢复误差。位置字段以毫秒记录，异常退出可能损失最近一个同步周期，倍速、网络延迟与失败重试会扩大视频时间滞后。初始恢复定位等待`seeked`完成，不把中间定位、重复`loadedmetadata`作为用户操作提交；用户拖动只在结束后提交目标位置。本批不增加本地每秒持久化。
...
第5.4轮阶段性实现使用同库只读查询投影，显式查询课程进度或分页查询我的课程时才汇总当前视频版本，不在心跳上报链路中重算课程，也不创建上述异步物化表。后续只有在真实查询负载证明需要时，才增加异步物化投影、重建及版本切换补偿；本阶段设计和验收见 `docs/acceptance/round-5.4-course-learning.md`。

## 8. 读写流程

### 写入

1. 验证登录、课程权益、视频版本、位置范围和上报区间大小。
2. Lua校验当前会话并续期，发布前读取已有断点；旧序号位置由后续缓存/数据库排序规则拒绝覆盖，事件仍可进入消费幂等链路。
3. 发布持久化 `VideoProgressReported` 消息并等待 publisher confirm，同时检查mandatory返回；超时结果不确定，复用原事件重试。
4. 尝试更新Redis乐观断点快照和最近学习ZSet，乐观快照不得覆盖相同代次/序号的已落库投影；缓存失败不改变MQ已接收结果。
5. 返回epoch、sequence、已知resume position，以及 `accepted`（MQ已确认接收）与 `cacheUpdated`；均不代表MySQL已经落库，cacheUpdated为false时前端不能用响应位置覆盖本地状态。
...
1. 校验课程权益及当前视频版本后读取Redis快照。
2. 未命中、读取故障或字段损坏则读取MySQL，尝试Lua回填，回填失败仍返回已落库断点；当前未增加互斥回填或随机TTL。
3. 数据库也没有记录时返回位置0，不缓存空值。
4. 最近学习索引为空或Redis故障时有界查询MySQL并过滤下架视频与失效权益；临近结尾从0重播或自动播放下一节仍属于前端产品规则，未在当前后端实现。

会话v2 Key同槽、升级时重新开播及故障边界见 `docs/acceptance/round-5.5-resume-hardening.md`。
...
- 客户端常规频率约每用户每视频 6 次/分钟；服务端允许暂停/拖动造成的小突发。观看区间达到20段或20秒时仍可提前批量提交，防止增量超过上限，常规心跳并非严格请求限流器。
